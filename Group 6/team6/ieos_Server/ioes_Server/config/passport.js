    ...
    function(req, name, password, done) {
        ...
        process.nextTick(function() {
            ...
            if (!req.user) {
                User.findOne({ 'local.name' :  name }, function(err, user) {
                    ...
                    if (user) {
                        ...
                        newUser.local.allow_devices = [];
                        newUser.local.permission = 'normal';
            ...
            // if the user is logged in but has no local account...
                ...
                // BUT let's check if the name used to connect a local account is being used by another user
                User.findOne({ 'local.name' :  name }, function(err, user) {
                    if (err)
                        return done(err);
                    
                    if (user) {
                        return done(null, false, req.flash('loginMessage', 'That name is already taken.'));
